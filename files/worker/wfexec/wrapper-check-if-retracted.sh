#!/bin/bash
# wrapper-check-if-retracted.sh
# Wrapper to safely run check-if-retracted.pl as the opencast user
# Usage: wrapper-check-if-retracted.sh <mediapackage-id> <output-file>

SCRIPT="/opt/opencast/wfexec/check-if-retracted.pl"
